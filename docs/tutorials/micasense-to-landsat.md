...
The title of this retained tutorial describes the project-level scientific
goal, not a direct software conversion. The relationship is **Drone → NEON →
Landsat**: NEON airborne hyperspectral products are the translating reference
between the drone observations and Landsat bandspace.

The local drone path does not invent NEON-style filenames, run the NEON
download workflow, or directly convolve drone inputs into Landsat bands.
Landsat-style products for NEON flightlines are generated by the standard
sensor harmonization stage and use names such as:
