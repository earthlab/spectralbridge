...
| BRDF model JSON | `<flight_id>_brdf_model.json` | Scene-level BRDF coefficient tables written before BRDF application. | Includes `iso`/`vol`/`geo` plus `ndvi_edges`, which records the realized NDVI bin boundaries for the coefficient rows rather than a standalone NDVI raster. |
